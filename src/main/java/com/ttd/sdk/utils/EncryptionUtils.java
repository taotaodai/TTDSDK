...
public class EncryptionUtils {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Hello经过MD5:" + EncryptionUtils.encryptToMD5(source));
        System.out.println("Hello经过SHA:" + EncryptionUtils.encryptToSHA(source));
        System.out.println("========随机生成Key进行加解密==============");
        // 生成一个DES算法的密匙
        SecretKey key = EncryptionUtils.createSecretDESKey();
        String str1 = EncryptionUtils.encryptToDES(key, source);
        System.out.println("DES加密后为:" + str1);
        // 使用这个密匙解密
        String str2 = EncryptionUtils.decryptByDES(key, str1);
        System.out.println("DES解密后为：" + str2);

        // 生成一个AES算法的密匙
        SecretKey key1 = EncryptionUtils.createSecretAESKey();
        String stra = EncryptionUtils.encryptToAES(key1, source);
        System.out.println("AES加密后为:" + stra);
        // 使用这个密匙解密
        String strb = EncryptionUtils.decryptByAES(key1, stra);
        System.out.println("AES解密后为：" + strb);
        System.out.println("========指定Key进行加解密==============");
        try {
            String AESKey = EncryptionUtils.getAESKey(EncryptionUtils.encryptToSHA(source));
            String DESKey = EncryptionUtils.getDESKey(EncryptionUtils.encryptToSHA(source));
            System.out.println(AESKey);
            System.out.println(DESKey);
            String str11 = EncryptionUtils.encryptToDES(DESKey, source);
            System.out.println("DES加密后为:" + str11);
            // 使用这个密匙解密
            String str12 = EncryptionUtils.decryptByDES(DESKey, str11);
            System.out.println("DES解密后为：" + str12);

            // 生成一个AES算法的密匙
            String strc = EncryptionUtils.encryptToAES(AESKey, source);
            System.out.println("AES加密后为:" + strc);
            // 使用这个密匙解密
            String strd = EncryptionUtils.decryptByAES(AESKey, strc);
